...
# Initial number to use for fragments that are existing the  domain
# Rather than hitting a dam. Exiting fragments will start counting from
# this number
fexit = 99900000
...
segments.plot(column='Frag', ax=ax, legend=True)
plt.axis([-71.5,-69,41,43])
#segments.plot(column='step', ax=ax[1], legend=True)
plt.show()
...
# %%
# segments.to_csv(run_name+'_look.csv') 
# fragments.to_csv(run_name+'_frag.csv') 
# fragments2.to_csv(run_name+'_frag2.csv') 